...
import type { IVariation } from '@eppo/js-client-sdk-common/dist/dto/variation-dto';
...
export function readMockRacResponse(): Record<
  string,
  IExperimentConfiguration
> {
  return JSON.parse(
    fs.readFileSync(TEST_DATA_DIR + MOCK_RAC_RESPONSE_FILE, 'utf-8')
  );
}
...
export function readAssignmentTestData(): IAssignmentTestCase[] {
  ...
  testCaseFiles.forEach((file) => {
    const testCase = JSON.parse(
      fs.readFileSync(ASSIGNMENT_TEST_DATA_DIR + file, 'utf8')
    );
